define([
    ...
    p.init = function () {
        ...
        this.data = {
            ...
            currentValue: this.settings.value,
            values: [this.settings.value],
            timeStamp: [new Date()]
        };
        this._lastIsFake = false;
        this._valueInitDone = false;
    ...
    p.updateBuffer = function (value) {
        // if we do not get a initial value from binding (because it's also 0 as the default) then simulate a change with the default value
        // so we get a first point in the graph
        if (!this._valueInitDone) {
            this.setValue(this.data.currentValue);
        }
        if (this._lastIsFake) {
            this.data.values.pop();
            this.data.timeStamp.pop();
            this._lastIsFake = false;
        }
        if (value === undefined) {
            value = _.last(this.data.values);
            if (value === undefined) {
                return;
            }
            this._lastIsFake = true;
        }
        this.data.values.push(value);
        this.data.timeStamp.push(new Date());
    ...
    function _dataInTimeSpan(data, timeStamp, timeSpan) {
        ...
        // always keep 1 value at > timespan so line never gets removed to early
        if (firstIdxInTimeSpan > 1) {
            return {
                valuesInSpan: data.slice(firstIdxInTimeSpan - 1),
                timeStampInSpan: timeStamp.slice(firstIdxInTimeSpan - 1)
            };
        } else {
            return {
                valuesInSpan: data,
